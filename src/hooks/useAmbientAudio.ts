...
export function useAmbientAudio(isActive: boolean) {
  ...
  }, [settings, isActive, stopAudio, startAudio, prefersReducedMotion]);
  ...
  const handleUserInteraction = useCallback(() => {
    ...
    if (isActive && settings.track !== 'silence' && !isPlaying && !audioRef.current) {
      startAudio(settings.track);
    }
  }, [isActive, settings.track, isPlaying, startAudio]);

  // Start/stop based on isActive
  useEffect(() => {
    if (isActive && settings.track !== 'silence') {
      hasUserInteractedRef.current = true; // Assume user navigated here intentionally
      startAudio(settings.track);
    } else if (!isActive) {
      stopAudio();
    }
  ...
  }, [isActive]); // Only depend on isActive, not on settings changes
